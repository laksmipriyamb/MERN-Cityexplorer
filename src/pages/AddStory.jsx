...
import { postStoryAPI } from "../server/allAPI";
...
export default function AddStory() {
  const [storyDetails, setStoryDetails] = useState({
    title: "", spotname: "", location: "", caption: "", uploadImage: null
  })
  ...
  const handlePostStory = async (e) => {
    e.preventDefault();
    const { title, spotname, location, caption, uploadImage } = storyDetails
    if (!title || !spotname || !location || !caption || !uploadImage) {
      ...
      if (token) {
        ...
        for (let key in storyDetails) {
          reqBody.append(key, storyDetails[key])
        }
        const result = await postStoryAPI(reqBody, reqHeader)
        console.log(result);
        if (result.status == 200) {
          toast.success("Your Story Posted Successfully...")
        } else if (result.status == 401) {
          toast.warning(result.response.data)
        } else {
          toast.error("Something went wrong!!!")
        }
        resetUploadStoryForm()

      }
    }
  }

  const resetUploadStoryForm = () => {
    setStoryDetails({
      title: "", spotname: "", location: "", caption: "", uploadImage: null
    })
    setPreviewUpload("")
    ...
    <HeadPortion/>
      {token ?
        <section className="relative min-h-screen overflow-hidden flex items-center justify-center px-6">
          ...
          {/* 🧾 GLASS CARD */}
          <div className="relative w-full max-w-3xl mt-28 rounded-[36px] bg-white/80 backdrop-blur-xl shadow-2xl p-8 md:p-12">
            ...
            <form className="space-y-6">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    value={storyDetails.spotname}
                    onChange={e => setStoryDetails({ ...storyDetails, spotname: e.target.value })}
                    className="w-full pl-5 pr-5 py-3 rounded-xl border bg-white/90 focus:ring-2 focus:ring-orange-400 outline-none transition"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={e => setStoryDetails({ ...storyDetails, location: e.target.value })}
              ...
              <div>
                ...
                <textarea
                  ...
                  value={storyDetails.caption}
                  onChange={e => setStoryDetails({ ...storyDetails, caption: e.target.value })}
              ...
              <div>
                ...
                {storyDetails.uploadImage && (
                  <div className="mt-4 rounded-2xl overflow-hidden">
                    <img
                      src={URL.createObjectURL(storyDetails.uploadImage)}
                      alt="preview"
